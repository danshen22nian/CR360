...
def main(args, cur_trainFile, cur_valFile, save_innerTimesPath, times):
    ...
    for epoch in range(epochs):
        ...
        print('[epoch %d] train_loss: %.3f  val_loss: %.3f sor: %.3f tt: %.3f top-1: %.3f' %
              (epoch + 1, running_loss / train_steps, val_mean_loss, sor, tt, top_1))
...
if __name__ == '__main__':
    ...
    save_path = r"E:\final_project\weights\ViT_New_1"
    val_fileNames = os.listdir(Val_path)
    val_fileNames = ["transalNet"]
    aug_times = [3]
    for i in range(0, len(val_fileNames)):
        ...
        inner_valNames = ["transalnet_new_REL_intraSim08"]
        for j in range(0, len(inner_valNames)):
            # 360Cat_NewDataset\whole_testResults
            cur_valFile = os.path.join(Val_path, val_fileNames[i], inner_valNames[j], "360Cat_NewDataset",
                                       "whole_testResults")
            save_innerPath = os.path.join(save_bigPath, inner_valNames[j])

            # for aug_index in range(0, 3):
            for aug_index in range(0, len(aug_times)):
                # times = aug_index + 1
                times = aug_times[aug_index]
